...
@implementation IFLYQRCodeUtils
#pragma mark 生成二维码
/**
 *  生成二维码图片
 *
...
+(UIImage *)createQRimageString:(NSString *)QRString sizeWidth:(CGFloat)sizeWidth fillColor:(UIColor *)color{
    CIImage *ciimage = [IFLYQRCodeUtils createQRForString:QRString];
    UIImage *qrcode = [IFLYQRCodeUtils createNonInterpolatedUIImageFormCIImage:ciimage withSize:sizeWidth];
    if (color) {
        CGFloat R, G, B;
        ...
        UIImage *customQrcode = [IFLYQRCodeUtils imageBlackToTransparent:qrcode withRed:R andGreen:G andBlue:B];
        return customQrcode;
    }
