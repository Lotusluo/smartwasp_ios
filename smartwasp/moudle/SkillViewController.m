...
//发现页数据
@property (nonatomic,strong) SkillerBean *findBean;
//刷新控件
...
static NSString *const ID1 = @"SkillerHeaderCell";
static NSString *const ID2 = @"SkillBannerCell";
...
-(void)reloadData:(DeviceBean *)device{
    ...
    if(device){
        ...
        [[NetDAO sharedInstance] getBos:bosAPI callBack:^(BaseBean<SkillerBean*> * _Nonnull cData) {
            ...
            if(!cData.errCode){
                SELF.findBean = [SkillerBean yy_modelWithJSON:cData.data];
                if(SELF.findBean){
                    //有发现页数据才算成功
                    [SELF loadDataSucess];
                    return;
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [imageView sd_setImageWithURL:[NSURL URLWithString:self.findBean.banners[index].image] placeholderImage:placeholderImage];
    return cell;
}
